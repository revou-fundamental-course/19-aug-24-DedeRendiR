...
function replacename() {
      let name = prompt("Please Enter Your Name please...", "");
...
function showslide(n) {
    let listImage = document.getElementsByClassName('MainArticle-Banner-Content');
...
function submitFeedback(e) {  
    ...
    //untuk waktu realtime saat submit
    var dateString = new Date().toLocaleString("en-US", {timeZone: "Asia/Jakarta"});
    var formattedString = dateString.replace(", ", " - ");
    const pesanvalue = pesan.value;
    //untuk mengambil data jenis kelamin
    ...
    if (namevalue =='' || datevalue == '' || gendervalue == '' || pesanvalue == '') {
        ...
        //untuk menambahkan komen di massage
...
function addFeedback(item){
    ...
    div.innerHTML = `
      ...
      <div class="comment_info">
        <small class="names">
         Nama: ${item.names}</small>
        <samall class="date">Tanggal Lahir: ${item.dates}</samall>
        <small class="gender">Jenis Kelamin: ${item.genders}</small>
        <small class="clock">Current Time: ${item.clock}</small>
        <p class="comment"> ${item.pesans}</p>
      </div>
    `;
